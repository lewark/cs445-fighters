...
def train_model(model_class, env_class, model_options: dict[str, Any], n_procs: int = 4, n_stack: int = 4, total_timesteps: int = 25000):
    env = None
    try:
        start_time = time.time()
        env = SubprocVecEnv([lambda: Monitor(env_class(), LOG_DIR) for proc in range(n_procs)])
    ...
    hyper_ps = [str(model_options[key]) for key in model_options] + [str(mean_reward)]
