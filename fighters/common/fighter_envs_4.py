...
class StreetFighter(FighterEnv):
    ...
    def compute_score(self, info: dict[str, Any]) -> int:
        ...
        if info.get("score", 0) > self.score:
            reward += (info.get("score", 0) - self.score) / 100000
        ...
        if info.get("enemy_health", 0) < self.enemy_health:
            reward += (self.enemy_health - info.get("enemy_health", 0)) / 100
        self.enemy_health = info.get("enemy_health", 0)
        
        if info.get("matches_won", 0) > self.r_wins:
            reward += 0.9 * info.get("matches_won", 0)
        self.r_win = info.get("matches_won", 0)
        
        if info.get("enemy_matches_won", 0) > self.enemy_wins:
            reward += -0.9 * info.get("enemy_matches_won", 0)
...
def train_model(model_class, env: Env, model_options: dict[str, Any], total_timesteps: int = 25000):
    ...
    model = model_class(env=env, verbose=0, device=device, tensorboard_log="results/board", **model_options)
    print("Learning...")
