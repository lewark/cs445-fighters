...
class StreetFighter(FighterEnv):
    ...
    # Overriden: Removal of round wins
    def compute_score(self, info: dict[str, Any]) -> int:
        return (
            info.get("score", 0) / 10  # Score 1000 per round win, reward 100 per round win 
            + (info.get("health", 0) - info.get("enemy_health", 0)) * 10  # Health = 144, Reward for Perfect = 1440
            + (info.get("matches_won", 0) - info.get("enemy_matches_won", 0)) * 1000  # 1 match win = 1000
        )
...
def train_model(model_class, env: Env, model_options: dict[str, Any], total_timesteps: int = 25000):
    ...
    print("Evaluating...")
    ep_rewards, ep_stds = evaluate_policy(model, env, n_eval_episodes=5, return_episode_rewards = True, deterministic = False)
    reward_mean = np.mean(np.array(ep_rewards))
    std_mean = np.mean(np.array(ep_stds))
    reward_sum = np.sum(np.array(ep_rewards))

    elapsed_time = time.time() - start_time
    hyper_ps = [str(model_options[key]) for key in model_options] + [str(ep_rewards), str(ep_stds), str(reward_mean), 
                                                                     str(std_mean), str(reward_sum), str(elapsed_time)]
    ...
    del model, env, ep_rewards, reward_mean, reward_sum, ep_stds
